...
const createStoreWithMiddelware = applyMiddleware( promiseMiddelware, ReduxThunk )(createStore);//그냥 store는 객체만 받을 수 있으므로 promise와 function도 받을 수 있도록 promise, thunk 미들웨어도 함께 만들어준다.
...
ReactDOM.render( //react-redux 에서 provider를 가져와서 app을 감싸면 이 어플리케이션과 리덕스가 연결이 된다
  <provider
		store = { createStoreWithMiddelware(Reducer,
				window.__REDUX_DEVTOOLS_EXTENSION__ &&  //redux extension을 구글에서 설치하고 이거 치면 사용 가능
